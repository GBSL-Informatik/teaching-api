...
import { ALL_DOCUMENT_ID, RO_DOCUMENT_ID, RW_DOCUMENT_ID, TEST_DOCUMENT_ID, documents as seedDocuments } from './seed-files/documents';
import { ALL_TEST_USERS_GROUP_ID, ALL_USERS_GROUP_ID, RO_GROUP_ID, RW_GROUP_ID, groups as seedGroups } from './seed-files/groups';
...
async function main() {
            ...
            data: [
                {
                    ...
                    documentId: RW_DOCUMENT_ID, /** `Shared Read/Write for ${USER_EMAIL}, Read Only for test users` */
                    readAccess: 'STUDENT',
                    writeAccess: 'ADMIN'
                },
                {
    ...
    await prisma.usersOnGroups.createMany({
        data: [
            ...[FOO_BAR_ID, TEST_USER_ID, USER_ID].filter((id) => !!id)
                .map((id) => ({
                    userId: id!,
                    groupId: ALL_USERS_GROUP_ID,
                    role: Role.ADMIN
                })),
            ...[FOO_BAR_ID, TEST_USER_ID].filter((id) => !!id)
                .map((id) => ({
                    userId: id!,
