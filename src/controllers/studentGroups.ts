...
export const addUser: RequestHandler<{ id: string; userId: string }, any, any> = async (req, res, next) => {
    const model = await StudentGroup.addUser((req as any).user!, req.params.id, req.params.userId);
    res.notifications = [
        {
            event: IoEvent.CHANGED_RECORD,
            message: {
...
export const removeUser: RequestHandler<{ id: string; userId: string }, any, any> = async (
    ...
    const model = await StudentGroup.removeUser((req as any).user!, req.params.id, req.params.userId);
    res.notifications = [
        {
            event: IoEvent.CHANGED_RECORD,
            message: {
...
export const all: RequestHandler = async (req, res, next) => {
    const users = await StudentGroup.all((req as any).user!);
    res.json(users);
};

export const destroy: RequestHandler<{ id: string }> = async (req, res, next) => {
    const group = await StudentGroup.deleteModel((req as any).user!, req.params.id);
    res.notifications = [
        {
            event: IoEvent.DELETED_RECORD,
            message: {
